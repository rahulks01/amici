...
export const getChannelMembers = catchAsync(async (req, res, next) => {
  const { channelId } = req.params;
  const userId = req.userId;
...
export const leaveChannel = catchAsync(async (req, res, next) => {
  ...
  if (isAdmin) {
    if (channel.members.length === 0) {
      ...
      return res.status(200).json({
        message: "You have left the channel and a new admin has been assigned",
      });
    }
  } else {
    await Channel.findByIdAndUpdate(channelId, {
...
export const addMembersToChannel = catchAsync(async (req, res, next) => {
...
export const searchUsersForChannel = catchAsync(async (req, res, next) => {
